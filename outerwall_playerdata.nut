...
::PlayerBestTimeEncoreArray <- array(MAX_PLAYERS, array(ZONE_COUNT_ENCORE, 5000))
...
::PlayerMiscStats <-
[
	::PlayerMinutesPlayed <- array(MAX_PLAYERS, 0)
...
[
	...
	::PlayerEncoreStatus <- array(MAX_PLAYERS, false)
...
::ResetPlayerDataArrays <- function(player_index)
{
	...
	{
		if(i == 1 || i == 3) //special case: soundtrack list and equipped cosmetic
			setting[player_index] = 0;
		else
			setting[player_index] = false;
	}
}
...
::PlayerSaveGame <- function(client)
{
	...
	foreach(i, setting in PlayerSettings)
	{
		if(i != 2) //don't save encore status
...
//TODO: add escape characters before any commas or semicolons or backslash and add read exceptions for them
